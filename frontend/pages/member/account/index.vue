...
<template>
	<div>
		<Breadcrumb title="Profile" :pages="['Member Area']" />
					...
					<div class="card">
						...
						<div class="card-body">
							<div class="form-group">
								...
								<input name="name" placeholder="Masukkan nama pengguna" :value="$user(`name`)" class="form-control" type="text" autocomplete="off" required />
								<span validation-for="name"></span>
							</div>
							<div class="form-group">
								<label>Username</label>
								<input name="username" placeholder="Masukkan username" :value="$user(`username`)" class="form-control" type="text" autocomplete="off" required />
								<span validation-for="username"></span>
							</div>
							<div class="form-group">
								<label>Email</label>
								<input name="email" placeholder="Masukkan email" :value="$user(`email`)" class="form-control" type="text" autocomplete="off" required />
							...
							<div class="form-group">
								<label>Alamat</label>
								<textarea name="address" placeholder="" class="form-control" rows="4" v-model="address"></textarea>
        ...
        async form_submit() {
            if (typeof jQuery !== "undefined") {
                try {
                    ...
                    const { data: response } = await this.$axios.post(`member/profile`, formData, {
                        headers: {
