...
<template>
	<div>
		<Breadcrumb title="Webinar" :pages="['Member Area']" />
			...
			<li v-for="(item, index) in results" :key="index">
				<div class="card overflow-hidden waves-effect w-100">
                    ...
                    <div class="card-footer">
						<NuxtLink :to="`/member/webinars/open/${item.code}`" class="btn btn-primary w-100"><i class="mdi mdi-video"></i> Lihat Webinar</NuxtLink>
                    </div>
				</div>
			</li>
		</ul>
	...
	methods: {
		async getData() {
			try {
				...
				const { data: response } = await this.$axios.get(`member/webinars`, {
					params: {
						page: this.page,
					},
				});
