...
<template>
	<div>
		<Breadcrumb title="Merchandise" :pages="['Member Area']" />
			...
			<li v-for="(item, index) in results" :key="index">
				<div class="card overflow-hidden waves-effect w-100">
                    ...
                    <div class="card-footer">
						<a target="_blank" :href="item.url" class="btn btn-primary w-100"><i class="mdi mdi-cart-arrow-down"></i> Belanja Sekarang</a>
                    </div>
				</div>
			</li>
		</ul>
	...
	methods: {
		async getData() {
			try {
				...
				const { data: response } = await this.$axios.get(`member/merchandises`, {
					params: {
						page: this.page,
					},
				});
