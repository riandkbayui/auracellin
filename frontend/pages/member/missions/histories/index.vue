...
<template>
	<div>
		<Breadcrumb title="Riwayat" :pages="['Member Area', 'Misi']" />
		...
		<ul v-if="results.length>0" class="list-unstyled m-0 p-0 mb-3">
			<li v-for="(item, index) in results" :key="index">
				<div class="card">
					...
					<div v-if="item.status != 'success'" class="card-footer">
						<NuxtLink :to="`/member/missions/open/${item.code}`" class="btn btn-primary w-100">
							<span class="text-white"><i class="fa fa-book-open"></i> Buka Misi</span>
						</NuxtLink>
					</div>
				</div>
			</li>
	...
	methods: {
		async getData() {
			try {
				...
				const { data: response } = await this.$axios.get(`member/missions/histories`, {
					params: {
						page: this.page,
					},
				});
