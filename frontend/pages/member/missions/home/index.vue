...
import Card from "@/components/member/missions/card";
</script>

<template>
	<div>
		<Breadcrumb title="Daftar Misi" :pages="['Member Area', 'Misi']" />
	...
	methods: {
		async getData() {
			try {
				...
				const { data: response } = await this.$axios.get(`member/missions`, {
					params: {
						page: this.page,
					},
				});
