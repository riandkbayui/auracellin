...
<template>
    <div>
        <Breadcrumb title="Edit" :pages="['Admin Area', 'Slideshow']" />
...
<script type="text/javascript">
    export default {
        ...
        async asyncData({$axios, params, error}) {
            try {
                const { data: response } = await $axios.get(`admin/slideshows/edit/${params.slug}`);
                return response;
            } catch (err) {
                console.log(err);
                let err_msg = '';
            ...
            async form_submit(e) {
                if (typeof jQuery !== "undefined") {
                    try {
                        ...
                        const { data: response } = await this.$axios.post(`admin/slideshows/edit/${slug}`, formData, {
                            headers: {
