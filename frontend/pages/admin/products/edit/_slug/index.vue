...
<template>
    <div>
        <Breadcrumb title="Edit" :pages="['Admin Area', 'Produk']" />
        ...
        <form action="" v-on:submit.prevent="form_submit" method="post">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group">
                        <label>Harga</label>
                        <input name="price" v-model="form.price" placeholder="Masukkan harga" value="" class="form-control" type="number" autocomplete="off" required>
                        <span validation-for="price"></span>
                    </div>
                    <div class="form-group">
                        <label>Deskripsi</label>
                        <textarea name="description" v-model="form.description" placeholder="Masukkan deskripsi" class="form-control" rows="6" required></textarea>
...
<script type="text/javascript">
    export default {
        ...
        async asyncData({$axios, params, error}) {
            try {
                const { data: response } = await $axios.get(`admin/products/edit/${params.slug}`);
                return response;
            } catch (err) {
                console.log(err);
                let err_msg = '';
            ...
            async form_submit(e) {
                if (typeof jQuery !== "undefined") {
                    try {
                        ...
                        const { data: response } = await this.$axios.post(`admin/products/edit/${slug}`, formData, {
                            headers: {
