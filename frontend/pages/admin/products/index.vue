...
<template>
	<div>
		<Breadcrumb title="Produk" :pages="['Admin Area']" />

		<div class="text-end mb-4">
			<NuxtLink class="btn btn-success bg-success bg-secondary" to="/admin/products/add">
			...
			<li v-for="(item, index) in results" :key="index">
				<div class="card w-100 mb-2 overflow-hidden">
					...
					<div class="card-body">
						...
						<p class="text-warning text-opacity-75 fsz-14 mb-1">Harga: {{ $idr(item.price, "Rp.") }}</p>
					...
					<div class="card-footer">
						<div class="d-flex gap-2">
							<NuxtLink class="btn btn-primary waves-effect w-100" :to="`/admin/products/edit/${item.id}`">
	...
	methods: {
				...
				Swal.fire({
					...
					if (result.isConfirmed) {
						try {
							...
							const { data: response } = await this.$axios.post(`admin/products/delete`, data, {
								headers: {
		...
		async getData() {
			try {
				...
				const { data: response } = await this.$axios.get(`admin/products`, {
					params: {
						page: this.page,
					},
				});
