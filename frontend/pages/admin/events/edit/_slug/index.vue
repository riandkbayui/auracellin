...
<template>
    <div>
        <Breadcrumb title="Edit" :pages="['Admin Area', 'Event']" />
...
<script type="text/javascript">
    export default {
        ...
        async asyncData({$axios, params, error}) {
            try {
                const { data: response } = await $axios.get(`admin/events/edit/${params.slug}`);
                return response;
            } catch (err) {
                console.log(err);
                let err_msg = '';
            ...
            async form_submit(e) {
                if (typeof jQuery !== "undefined") {
                    try {
                        ...
                        const { data: response } = await this.$axios.post(`admin/events/edit/${slug}`, formData, {
                            headers: {
