...
<template>
    <div>
        <Breadcrumb title="Tambah Client" :pages="['Admin Area', 'Iklan Terima Beres']" />
            ...
            async form_submit(e) {
                if (typeof jQuery !== "undefined") {
                    try {
                        ...
                        const { data: response } = await this.$axios.post(`admin/itbs/client-add/${slug}`, formData, {
                            headers: {
                        ...
                        Swal.fire({
                            ...
                            this.$router.push(response.redirect_to);
                        });
                    } catch (err) {
                        console.log(err);
                        let err_msg = '';
