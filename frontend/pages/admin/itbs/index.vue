...
<template>
	<div>
		<Breadcrumb title="Iklan Terima Beres" :pages="['Admin Area']" />

		<div class="mt-2 mb-3 text-end">
            <NuxtLink class="btn btn-success bg-success bg-gradient" :to="`/admin/itbs/add`"><i class="mdi mdi-plus-circle"></i> Buat Baru</NuxtLink>
        </div>

		<ul v-if="results.length>0" class="list-unstyled m-0 p-0 mb-3">
			<li v-for="(item, index) in results" :key="index">
                <NuxtLink :to="`/admin/itbs/edit/${item.id}`">
	...
	methods: {
		async getData() {
			try {
				...
				const { data: response } = await this.$axios.get(`admin/itbs`, {
					params: {
						page: this.page,
					},
				});
