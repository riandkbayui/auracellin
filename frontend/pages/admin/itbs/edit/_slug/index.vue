...
<template>
    <div>
        <Breadcrumb title="Edit" :pages="['Admin Area', 'Iklan Terima Beres']" />
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description" placeholder="Masukkan deskripsi" v-model="form.description" class="form-control" rows="6"></textarea>
                        <span validation-for="description"></span>
                    </div>
                    <div class="form-group">
                        <label>Member</label>
                        <select name="user_id" id="user_id" class="form-control">
                            <option v-if="form.user_id!=''" :value="form.user_id" selected>{{user.username}}</option>
                        </select>
...
<script type="text/javascript">
    export default {
        ...
        async asyncData({$axios, params, error}) {
            try {
                const { data: response } = await $axios.get(`admin/itbs/edit/${params.slug}`);
                return response;
            } catch (err) {
                console.log(err);
                let err_msg = '';
            ...
            async form_submit(e) {
                if (typeof jQuery !== "undefined") {
                    try {
                        ...
                        const { data: response } = await this.$axios.post(`admin/itbs/edit/${slug}`, formData, {
                            headers: {
