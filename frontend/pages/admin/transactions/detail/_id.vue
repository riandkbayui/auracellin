...
<template>
	<div>
		<Breadcrumb title="Detail Transaksi Admin" :pages="['Admin Area', 'Transaksi', 'Detail']" />
		...
		<div>
			<div class="card">
				...
				<div class="card-body">
					...
					<div class="d-flex justify-content-between mb-2">
						...
						<span class="badge bg-warning text-capitalize h-100">{{ transaction.status }}</span>
					</div>
			...
			<div class="card">
				<div class="card-header">
					<h5 class="card-title text-white">Update Status & Resi</h5>
				</div>
				<div class="card-body">
					<form @submit.prevent="updateStatus">
						...
						<div class="form-group mb-3">
							...
							<input type="text" v-model="form.tracking_number" class="form-control" placeholder="Masukkan nomor resi..." />
						</div>
	...
	methods: {
		async getDetail() {
			...
			try {
				...
				const { data: response } = await this.$axios.get(`admin/transactions/detail/${id}`);
				this.transaction = response.transaction;
				this.details = response.details;
				this.city = response.city;
				this.form.status = this.transaction.status;
				this.form.tracking_number = this.transaction.tracking_number;
			} catch (err) {
		...
		async updateStatus() {
			...
			try {
				...
				data.append("tracking_number", this.form.tracking_number || "");

				const { data: response } = await this.$axios.post(`admin/transactions/updateStatus/${id}`, data, {
					headers: {
