...
class Transactions extends BaseServices {

	public $model;

	public function __construct() {
		parent::__construct();
		$this->model = model("TransactionsModel");
	}
	...
	public function findOne($filter="") {
		if (is_array($filter)) {
			...
			$this->model->where('transactions.id', $filter);
		}
	...
	public function createTransaction($data) {
		...
		try {
			...
			foreach ($products as $p) {
				$product = service("Products")->findOne($p['product_id']);
			...
			$transaction->invoice      = "TRX-" . $data['user_id'] . "-" . date('Ymd') . "-" . str_rand(4);
			$transaction->total        = $totalAmount;
			$transaction->city_id      = $data['city_id'];
			$transaction->full_address = $data['full_address'];
			$transaction->description  = $data['description'];
			$transaction->status       = 'pending';

			$payment_photo = $data['payment_photo'];
			if ($payment_photo && $payment_photo->isValid()) {
				$file_name = upload_img($payment_photo, "uploads/payments/");
				$transaction->payment_photo = "uploads/payments/" . $file_name;
			}

			$transaction_id = $this->create($transaction);

			// Save Transaction Details
			foreach ($transactionDetails as $detail) {
				$detail['transaction_id'] = $transaction_id;
				service("TransactionDetails")->create($detail);
			}
