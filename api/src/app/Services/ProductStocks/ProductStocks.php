...
namespace App\Services\ProductStocks;

use App\Services\BaseServices;
use Exception;

class ProductStocks extends BaseServices {

	public $model;

	public function __construct() {
		parent::__construct();
		$this->model = model("ProductStocksModel");
	}
	...
	public function findOne($filter="") {
		if (is_array($filter)) {
			...
			$this->model->where('product_stocks.id', $filter);
		}
    ...
    public function getStock($product_id) {
        $data = $this->findOne([
            ["select", ["SUM(CASE WHEN type = 'in' THEN qty ELSE -qty END) as total_stock"]],
            ["where", "product_id", $product_id]
        ]);
        return $data && $data->total_stock ? (int) $data->total_stock : 0;
    }
