...
class Tutorials  extends BaseController {
    ...
    private $upload_path = "uploads/tutorials/";
    ...
    public function getIndex() {
        try {
            ...
            $getTotal = service("Tutorials")->findOne(array_merge($params, [
                ["selectCount", "tutorials.id", "total"]
            ]));

            $params[] = ["orderBy", "tutorials.id", "desc"];
            $params[] = ["select", ["tutorials.id", "tutorials.name", "tutorials.photo", "tutorials.description", "tutorials.status"]];
            $params[] = ["limit", $limit, $offset];
            $results = service("Tutorials")->findAll($params);
    ...
    public function postAdd(){
        ...
        try {
            if ($this->validate($rules)) {
                ...
                service("Tutorials")->create($obj);

                return $this->response->setJSON([
                    "message" => "Tutorial ditambahkan."
                ]);
            } else {
    ...
    public function getEdit($id){
        try {
            $id = numeric($id);
            $form = service("Tutorials")->findOne([
    ...
    public function postEdit($id){
        ...
        try {
            ...
            if ($this->validate($rules)) {
                $tutorial = service("Tutorials")->findOne([
                    ["where", "id", numeric($id)]
                ]);
                ...
                service("Tutorials")->update($tutorial->id, $obj);

                return $this->response->setJSON([
                    "message" => "Tutorial diperbarui."
                ]);
            } else {
                return $this->response->setStatusCode(400)->setJSON([
                    "message" => lang("Validation.invalid"),
    ...
    public function postDelete(){
        try {
            ...
            service("Tutorials")->delete($tutorial->id);
            return $this->respond([
                "message" => "Tutorial dihapus"
            ]);
        } catch (\Throwable $th) {
            return $this->respond([
                "message" => $th->getMessage()
